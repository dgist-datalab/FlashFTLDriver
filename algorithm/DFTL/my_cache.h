...
typedef struct my_cache{
	...
	bool (*is_needed_eviction)(struct my_cache *, uint32_t lba, uint32_t* prefetching_num, uint32_t eviction_hint);
	...
	uint32_t (*update_eviction_hint)(struct my_cache *, uint32_t lba, uint32_t eviction_hint, bool increase);
	...
	uint32_t (*insert_entry_from_translation)(struct my_cache *, GTD_entry *, uint32_t lba, char *data, uint32_t *prefetching_num, uint32_t *eviction_hint, uint32_t org_eviction_hint);
	...
	struct mapping_entry *(*get_eviction_mapping_entry)(struct my_cache *, uint32_t lba);//if return value is NULL, it is clean eviction.
	bool (*update_eviction_target_translation)(struct my_cache* , uint32_t lba, GTD_entry *etr, mapping_entry *map, char *data);
