...
uint32_t fine_update_eviction_hint(struct my_cache *, uint32_t lba, uint32_t evicition_hint, bool increase);
...
uint32_t fine_insert_entry_from_translation(struct my_cache *, GTD_entry *, uint32_t lba, char *data, uint32_t *, uint32_t *eviction_hint, uint32_t);
