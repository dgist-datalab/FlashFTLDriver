...
plt.scatter(x_total,valid_rate_list, color='black')
plt.ylim([0.2, 0.85])
plt.axhline(total_rate_avg,color='r')
#plt.show()
plt.savefig(valid_path+"_total.png")
fig.clear()
...
plt.show()
plt.savefig(valid_path+"_group.png")
...
for i in range(group_num):
    ...
    print("group "+str(i+1)+" rate avg: ", str(round(group_rate_avg[i]*100,3))+"%")
print(" ")
print("total rate avg:", str(round(total_rate_avg*100,3))+"%")
