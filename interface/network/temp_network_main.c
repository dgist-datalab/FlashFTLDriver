...
void *serv_end_req(algo_req *req) {
	...
	/*
	measure_calc(&req->server_time);
	int slot_num=req->server_time.micro_time/TIMESLOT;
...
//	setsockopt(clnt_fd2, IPPROTO_TCP, TCP_NODELAY, (const char *)&option, sizeof(option));
	...
	for(int i=0; i<1000000/TIMESLOT+1; i++){
		...
		printf("%d\t%ld\t%f\n",i * 10,read_cdf[i],(float)cumulate_number/cnt);
		if(cnt==cumulate_number)
			break;
	}	
